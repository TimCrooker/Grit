...
export async function runCLI(): Promise<void> {
	...
	cli.command('[generator] [outDir]', 'Run a generator')
	...
	cli.command('set-alias <name> <value>', 'Set an alias for a generator path')
		.option('-h, --help', 'Display CLI usages')
		.action((name, value) =>
			import('./cmd/set-alias').then((res) => res.setAlias()(name, value))
		)

	cli.command('get-alias <name>', 'Get the generator for an alias')
		.option('-h, --help', 'Display CLI usages')
		.action((name) =>
			import('./cmd/get-alias').then((res) => res.getAlias(cli)(name))
		)

	cli.command('list', 'List all downloaded generators')
		.option('-h, --help', 'Display CLI usages')
